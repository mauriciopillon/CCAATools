...
pAno1 <- read_excel(planilha, sheet=sprintf("%s",ano1))
pAno2 <- read_excel(planilha, sheet=sprintf("%s",ano1+1))
pAno3 <- read_excel(planilha, sheet=sprintf("%s",ano1+2))
pAno4 <- read_excel(planilha, sheet=sprintf("%s",ano1+3))

#remover submenu, B5 (periodico e eventos), valores Qualis 2016
#AJ=36: alterar se for acrescentado um novo nome de professor
pAno1 <- pAno1[c(-1, -10, -20),seq (3, length(colnames(pAno1)), 2)*-(1)]
pAno2 <- pAno2[c(-1, -10, -20),seq (3, length(colnames(pAno2)), 2)*-(1)]
pAno3 <- pAno3[c(-1, -10, -20),seq (3, length(colnames(pAno3)), 2)*-(1)]
pAno4 <- pAno4[c(-1, -10, -20),seq (3, length(colnames(pAno4)), 2)*-(1)]

#Carla (coluna 7) e Cristina (coluna 9)
pAno1 <- pAno1[,c(-7,-9)]
pAno2 <- pAno2[,c(-7,-9)]
pAno3 <- pAno3[,c(-7,-9)]
pAno4 <- pAno4[,c(-7,-9)]
...
profnames <- c ("Adriano", "André", "Avanilde", "Charles", "Fabiano", "Guilherme",
                "Isabela", "Janine", "Marcelo", "Maurício", "Rafael P.",
                "Rafael O.", "Roberto", "Yuri")
...
#for (tprof in 1:5) {
  ...
  # nrper <- sum (pAno4[1:8,tprof],pAno1[1:8,tprof],pAno2[1:8,tprof],pAno3[1:8,tprof])
  # somente periódico restrito 1:4 (A1, A2, A3, A4)
  print (profnames[tprof])
  nrper <- sum (as.numeric(unlist(pAno1[1:4,(tprof+3)])),
                as.numeric(unlist(pAno2[1:4,(tprof+3)])),
                as.numeric(unlist(pAno3[1:4,(tprof+3)])),
                as.numeric(unlist(pAno4[1:4,(tprof+3)])))
  nevt <- as.numeric (nrper * 3)
  sop <<- data.frame(
    pAno1[1:length(vtposicao),(tprof+3)],
    pAno2[1:length(vtposicao),(tprof+3)],
    pAno3[1:length(vtposicao),(tprof+3)],
    pAno4[1:length(vtposicao),(tprof+3)])
...
bp <- barplot2(as.numeric(vtall[order(as.numeric(vtall[,2])),2]),
               ...
               #col = cor,
               main = c("Pontuação Produção Completa ",periodo_avaliacao)#,
               #ylim = c(0, 2000)
)
...
bp <- barplot2(as.numeric(ttcapes[order(as.numeric(ttcapes[,2])),2]),
               ...
               main = c("TopCapes ",periodo_avaliacao)#,
               #ylim = c(0, 2000)
)
...
bp <- barplot2(as.numeric(vtprofpt5[order(as.numeric(vtprofpt5[,2])),2]),
               ...
               main = c("Pontuação Top5", periodo_avaliacao) #,ylim = c(0, 500)
)
...
bp <- barplot2(as.numeric(vtprofpt10[order(as.numeric(vtprofpt10[,2])),2]),
               ...
               main = c("Pontuação Top10", periodo_avaliacao)#, ylim = c(0, 900)
)
...
bp <- barplot2(as.numeric(vtpub3par1[order(as.numeric(vtpub3par1[,2])),2]),
               ...
               main = c("Pontuação Pq1xPer. 3xEvt", periodo_avaliacao) #,ylim = c(0, 1700)
)
...
title("Médias Top5, Top10, Pq1x3 & Bottom (1-14)")
...
title("Medianas Top5, Top10, Pq1x3 & Bottom (1-14)")
legend("bottomright", c("Pq1x3", "Top5", "Top10", "TopCapes"), col = color, pch=plotchar, lty=linetype)
axis(1, at=sort (c(rep(1:16))),labels=c(rep(0:15)), las=1)
text(2,-1.2, planilha,cex=1,pos=1, xpd=TRUE)
#abline(v=3, col = "blue")
...
title(c("TopCapes", periodo_avaliacao))
...
boxplot(
        ...
        xlab = "Número de permanentes reclassificados (p/ colaborador + desligado)")
title(c("Top5 ", periodo_avaliacao))
grid (NA,10,col = "black")
axis(1, at=sort (c(rep(1:16))),labels=c(rep(0:15)), las=1)
...
boxplot(
        ...
        xlab = "Número de permanentes reclassificados (p/ colaborador + desligado)")
title(c("Top10", periodo_avaliacao))
grid (NA,10,col = "black")
axis(1, at=sort (c(rep(1:16))),labels=c(rep(0:15)), las=1)
...
boxplot(
        ...
        xlab = "Número de permanentes reclassificados (p/ colaborador + desligado)")
grid (NA,10,col = "black")
title(c("TopPq13E", periodo_avaliacao))
axis(1, at=sort (c(rep(1:16))),labels=c(rep(0:15)), las=1)
